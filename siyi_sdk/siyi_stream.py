...
class SIYISTREAM:
    ...
    def test_continuous(self):
        """
        Test the stream with FPS and Resolution data
        """
        vcap = cv2.VideoCapture(self._stream_link)
        while(1):
            ret, frame = vcap.read()
            # Diplay frame resolution and frame rate on top left corner of frame
            cv2.putText(frame, "Resolution: {}".format(frame.shape), (0, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
            cv2.putText(frame, "FPS: {}".format(vcap.get(cv2.CAP_PROP_FPS)), (0, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
    ...
    def test_latest(self):
        """
        Tests the stream by displaying the latest frame at 1hz
        """
        self.connect()
